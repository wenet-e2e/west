...
def save_audio(audio_data, sample_rate, to_dir='./result/chat/wavs', to_name=None):
    # 获取当前日期和时间
...
class ChatHistory(object):
    ...
    def add_speech_history(self, speech):
        ...
        speech = load_wav(speech, target_sr=self.extractor.sampling_rate, device='cuda')
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Root path to the goat_slm", required=True
    )
    ### args for generation
    parser.add_argument(
    ...
    parser.add_argument(
        "--min_new_tokens", type=int, default=10,
    ...
    parser.add_argument(
        "--max_turn_num", type=int, default=6,
        help="max turn num"
    )
    parser.add_argument(
        "--cache-dir", type=str, default="/tmp/TeleAdapterSLM", help="Cache directory."
    )
...
def gradio_answer(chatbot, enable_thinking, is_his_empty_think, is_speech_generate_run, do_sample, max_turn_num, max_new_tokens, sys_prompt):
    generation_config.update(
        **{
            'do_sample': do_sample,
    ...
    print('************************* print history end!! ******************************')

    text_tokens, speech_units = Y_model.chat(
        history=history.history,
        generation_config=generation_config,
        speech_llm_input_ids=torch.tensor(speech_llm_input_ids, dtype=torch.int, device="cuda").unsqueeze(0),
    ...
    if speech_units.size(-1) != 0:
        ...
        if enable_thinking and ('粤语' in response or '河南' in response or '东北' in response or '四川' in response or '上海' in response):
            output_units = output_units[17:] # + output_units[]
        print(f"response text len: {len(text_tokens[0])}, unit len: {len(output_units)//4}")
...
def add_file(chatbot, gr_audio, enable_thinking, model_type):
    ...
    if model_type == 'GOAT-SLM2-1.8B':
        ...
        history.add_text_history(f"<|im_start|>user\n")  # f"<|im_start|>user\n{transcription}"
        user_message = "<|im_end|>\n<|im_start|>assistant\n"
        if enable_thinking == False:
            user_message += '<think>\n\n</think>\n\n'
...
with gr.Blocks() as demo:
    ...
    chatbot = gr.Chatbot([], elem_id="chatbot", height=750, avatar_images=(None, (os.path.join(os.path.dirname(__file__), "data/goat.png"))))
    gr_model_type = gr.Textbox(value=Y_model.config.model_type, visible=False)
    with gr.Row():
        with gr.Column(scale=0.2, min_width=0):
            is_speech_generate_run = gr.Checkbox(label="speech generate run?", value=args.is_speech_generate_run)
            do_sample = gr.Checkbox(label="do sample?", value=True)
            enable_thinking = gr.Checkbox(label="enable thinking?", value=True, visible=Y_model.config.model_type=='GOAT-SLM2-8B')
            is_his_empty_think = gr.Checkbox(label="history empty_think?", value=False, visible=Y_model.config.model_type=='GOAT-SLM2-8B')
    ...
    txt_msg = txt.submit(add_text, [chatbot, txt, enable_thinking, gr_model_type], [chatbot, txt], queue=False).then(
        gradio_answer, [chatbot, enable_thinking, is_his_empty_think, is_speech_generate_run, do_sample, max_turn_num, max_new_tokens, sys_prompt], [chatbot]
    )
    txt_msg.then(lambda: gr.update(interactive=True), None, [txt], queue=False)

    file_msg = btn.upload(add_file, [chatbot, btn, enable_thinking, gr_model_type], [chatbot], queue=False).then(
        gradio_answer, [chatbot, enable_thinking, is_his_empty_think, is_speech_generate_run, do_sample, max_turn_num, max_new_tokens, sys_prompt], [chatbot]
    )

    input_audio_mic.change(add_micophone_file, [chatbot, input_audio_mic, enable_thinking, gr_model_type], [chatbot, input_audio_mic], queue=True).then(
        gradio_answer, [chatbot, enable_thinking, is_his_empty_think, is_speech_generate_run, do_sample, max_turn_num, max_new_tokens, sys_prompt], [chatbot]
    )
