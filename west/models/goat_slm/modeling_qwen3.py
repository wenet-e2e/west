        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.config._attn_implementation != "eager":
            if self.config._attn_implementation == "sdpa" and kwargs.get(
                ...
                logger.warning_once(
                    "`torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to "  # noqa
                    'eager attention. This warning can be removed using the argument `attn_implementation="eager"` when loading the model.'  # noqa
                )
            else:
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        if (config.sliding_window and config._attn_implementation
                != "flash_attention_2"):  # diff with Llama is this warning
            logger.warning_once(
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for index, decoder_layer in enumerate(
                self.layers[start_layer:self.config.num_hidden_layers]):
    ...
    def _update_causal_mask(
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if attention_mask is not None and past_key_values is not None:
                is_padding_right = attention_mask[:, -1].sum().item(
                ) != input_tensor.size()[0]
                if is_padding_right:
                    raise ValueError(
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if getattr(text_config, "use_sliding_window",
                       True) and text_config.sliding_window is not None:
                ...
                if not isinstance(
                        past_key_values,
                        SlidingWindowCache) or sequence_length > target_length:
...
@auto_docstring
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationWithCE, GenerationMixin):
