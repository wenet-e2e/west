...
from west.utils.mask import make_pad_mask
...
class InterpolateRegulator(nn.Module):
    ...
    def inference(self, x1, x2, mel_len1, mel_len2, input_frame_rate=50):
        # in inference mode, interploate prompt token and
        # token(head/mid/tail) seprately, so we can get a clear separation
        # point of mel
        # x in (B, T, D)
        if x2.shape[1] > 40:
        ...
        if x1.shape[1] != 0:
            x1 = F.interpolate(x1.transpose(1, 2).contiguous(),
                               size=mel_len1, mode='linear')
