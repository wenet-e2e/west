...
from transformers import AutoTokenizer, WhisperFeatureExtractor, AutoModelForCausalLM
...
def process_dataset(batch, tokenizer, type='qwen', enable_thinking=False):
    ...
    if 'qwen' in type:
        text_llm_prefix = f"<|im_start|>system\n{text_llm_system_prompt}<|im_end|>\n<|im_start|>user\n"
        speech_llm_prefix = f"<|im_start|>system\n{speech_llm_system_prompt}<|im_end|>\n<|im_start|>user\n"
        suffix = "<|im_end|>\n<|im_start|>assistant\n"
...
def main():
    ...
    assert (Y_model.config.model_type not in ('GOAT-SLM1-7B', 'GOAT-SLM2-1.8B')) or enable_thinking, \
    ...
    with open(f"{args.output_file}/output.json", "w") as fout:
        for index, data_ in enumerate(dataset):
            ...
            if speech_units.size(-1) != 0:
                ...
                if '粤语' in think or '河南' in think or '东北' in think or '四川' in think or '上海' in think:
