...
class WavReader {
 public:
  ...
  explicit WavReader(const std::string& filename, bool normalized = false) {
    Open(filename, normalized);
  }

  bool Open(const std::string& filename, bool normalized = true) {
    ...
    // Normalize to [-1, 1] if normalized is true
    if (normalized) {
      switch (bits_per_sample_) {
        case 8: {
          // 8-bit: range is 0-255, normalize to [-1, 1]
          for (int i = 0; i < num_data; ++i) {
            data_[i] = (data_[i] - 128.0f) / 128.0f;
          }
          break;
        }
        case 16: {
          // 16-bit: range is -32768 to 32767, normalize to [-1, 1]
          for (int i = 0; i < num_data; ++i) {
            data_[i] = data_[i] / 32768.0f;
          }
          break;
        }
        case 32: {
          // 32-bit: range is -2147483648 to 2147483647, normalize to [-1, 1]
          for (int i = 0; i < num_data; ++i) {
            data_[i] = data_[i] / 2147483648.0f;
          }
          break;
        }
      }
